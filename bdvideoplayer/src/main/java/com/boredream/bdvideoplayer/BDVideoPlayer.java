...
public class BDVideoPlayer {
    ...
    /**
     * 这个玩意儿一执行,那么就重新加载了,最多只执行一次就够了,不然从头开始放
     */
    ...
    /**
     * 后台切换回来继续播放执行
     * 相对于下面,仅仅重新实现了setOnPreparedListener监听里的逻辑
     * @param position
     */
    public void jixubofang(final int position){
//        player = new MediaPlayer();
        if (player== null || path == null || surfaceHolder == null) {
            // not ready for playback just yet, will try again later
            return;
        }
        try {
